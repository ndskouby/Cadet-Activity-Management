...
class Ripple extends BaseComponent {
  ...
  _autoInit(event) {
    ...
    if (
      ...
      dataAttributes.rippleInit === false
    ) {
      return;
    }
    ...
    if (this._element.tagName.toLowerCase() === "input") {
      ...
      if (
        ...
        parent.classList.contains(CLASSNAME_RIPPLE)
      ) {
    ...
    this._rippleTimer = setTimeout(() => {
      if (ripple) {
        ...
        if (this._element) {
          ...
          if (
            this._rippleInSpan &&
            this._element.classList.contains(CLASSNAME_RIPPLE_WRAPPER)
          ) {
  ...
  _removeOldColorClasses(target) {
    ...
    const PARENT_CLASSS_COLOR =
      target.classList.value.match(REGEXP_CLASS_COLOR) || [];
