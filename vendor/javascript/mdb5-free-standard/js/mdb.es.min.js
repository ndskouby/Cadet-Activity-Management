...
const isDisabled$1 = (element2) => {
  ...
  return (
    element2.hasAttribute("disabled") &&
    element2.getAttribute("disabled") !== "false"
  );
};
const getjQuery$1 = () => {
  const { jQuery } = window;
...
function getUidEvent(element2, uid) {
  return (
    (uid && `${uid}::${uidEvent$1++}`) || element2.uidEvent || uidEvent$1++
  );
}
...
function addHandler$1(
  ...
  const previousFn = findHandler$1(
    handlers,
    originalHandler,
  ...
  const uid = getUidEvent(
    originalHandler,
    originalTypeEvent.replace(namespaceRegex$1, ""),
  );
  const fn2 = delegation
    ? bootstrapDelegationHandler$1(element2, handler, delegationFn)
    : bootstrapHandler$1(element2, handler);
...
const Manipulator$1 = {
  ...
  getDataAttribute(element2, key) {
    return normalizeData$1(
      element2.getAttribute(`data-mdb-${normalizeDataKey$1(key)}`),
    );
  },
  offset(element2) {
...
const isVisible = (element2) => {
  ...
  const elementIsVisible =
    getComputedStyle(element2).getPropertyValue("visibility") === "visible";
...
function addHandler(
  ...
  const fn2 = isDelegated
    ? bootstrapDelegationHandler(element2, handler, callable)
    : bootstrapHandler(element2, callable);
...
const Manipulator = {
  ...
  getDataAttribute(element2, key) {
    return normalizeData(
      element2.getAttribute(`data-mdb-${normalizeDataKey(key)}`),
    );
  },
};
class Config {
  ...
  _mergeConfigObj(config, element2) {
    const jsonConfig = isElement$1(element2)
      ? Manipulator.getDataAttribute(element2, "config")
      : {};
    return {
...
let BaseComponent$1 = class BaseComponent extends Config {
  ...
  static getOrCreateInstance(element2, config = {}) {
    return (
      this.getInstance(element2) ||
      new this(element2, typeof config === "object" ? config : null)
    );
  }
  static get VERSION() {
    return VERSION;
...
let Button$1 = class Button extends BaseComponent$1 {
  ...
  toggle() {
    this._element.setAttribute(
      "aria-pressed",
      this._element.classList.toggle(CLASS_NAME_ACTIVE$5),
    );
  }
  // Static
...
const initComponent = (component, manualInit = false) => {
  ...
  const isToggler =
    (thisComponent == null ? void 0 : thisComponent.isToggler) || false;
  defineJQueryPlugin(component);
  if (thisComponent == null ? void 0 : thisComponent.advanced) {
    thisComponent.advanced(
      component,
      thisComponent == null ? void 0 : thisComponent.selector,
    );
    return;
  }
  if (isToggler) {
...
class InitMDB {
  constructor(defaultInitSelectors2) {
    ...
    __publicField(this, "initMDB", (components, checkOtherImports = false) => {
      const componentList = Object.keys(_defaultInitSelectors).map(
        (element2) => {
          ...
          if (requireAutoInit) {
            ...
            if (
              !component &&
              !InitializedComponents.get(element2) &&
              checkOtherImports
            ) {
              console.warn(
...
class Button2 extends Button$1 {
  constructor(element2) {
    ...
    if (this._element) {
      ...
      this._init();
      Manipulator$1.setDataAttribute(
        this._element,
  ...
  dispose() {
    ...
    Manipulator$1.removeDataAttribute(
      this._element,
      `${this.constructor.NAME}-initialized`,
    );
    super.dispose();
  ...
  _toggleVisibility(isVisible2) {
    ...
    const listTranslate = isVisible2
      ? "translate(0)"
    ...
    if (this._buttonListElements) {
      this._buttonListElements.forEach((el) =>
        Manipulator$1[action](el, CLASS_NAME_SHOWN),
      );
    }
...
class ScrollBarHelper {
  ...
  _setElementAttributes(selector, styleProperty, callback) {
    ...
    const manipulationCallBack = (element2) => {
      if (
        element2 !== this._element &&
        window.innerWidth > element2.clientWidth + scrollbarWidth
      ) {
        return;
      }
...
let Carousel$1 = class Carousel extends BaseComponent$1 {
  ...
  _updateInterval() {
    ...
    const elementInterval = Number.parseInt(
      element2.getAttribute("data-mdb-interval"),
      10,
    );
  ...
  return (
    (isElement(element2)
      ...
      : // $FlowFixMe[prop-missing]
        element2.document) || window.document
  ).documentElement;
}
...
function getTrueOffsetParent(element2) {
  if (
    !isHTMLElement(element2) || // https://github.com/popperjs/popper-core/issues/837
...
function getClippingParents(element2) {
  ...
  var canEscapeClipping =
    ["absolute", "fixed"].indexOf(getComputedStyle$1(element2).position) >= 0;
  var clipperElement =
...
let Tooltip$1 = class Tooltip extends BaseComponent$1 {
  ...
  show() {
    ...
    const showEvent = EventHandler.trigger(
      this._element,
      this.constructor.eventName(EVENT_SHOW$6),
    );
    ...
    const complete = () => {
      EventHandler.trigger(
        this._element,
        this.constructor.eventName(EVENT_SHOWN$6),
      );
  ...
  hide() {
    ...
    const hideEvent = EventHandler.trigger(
      this._element,
      this.constructor.eventName(EVENT_HIDE$6),
    );
    if (hideEvent.defaultPrevented) {
      return;
    }
    ...
    const complete = () => {
      ...
      EventHandler.trigger(
        this._element,
        this.constructor.eventName(EVENT_HIDDEN$6),
      );
    };
    this._queueCallback(complete, this.tip, this._isAnimated());
  }
  ...
  _isAnimated() {
    return (
      ...
      (this.tip && this.tip.classList.contains(CLASS_NAME_FADE$3))
    );
  }
  _isShown() {
...
class ScrollSpy2 extends ScrollSpy$1 {
  ...
  _hide(target) {
    const itemsToHide = SelectorEngine$1.findOne(
      SELECTOR_LIST,
      target.parentNode,
    );
  ...
  _getCollapsibles() {
    const collapsibleElements = SelectorEngine$1.find(
  ...
  _hideSubsection() {
    const unactives = SelectorEngine$1.find(
      SELECTOR_COLLAPSIBLE_SCROLLSPY,
    ).filter((collapsible) => {
...
let Tab$1 = class Tab extends BaseComponent$1 {
  ...
  show() {
    ...
    const hideEvent = active
      ? EventHandler.trigger(active, EVENT_HIDE$5, { relatedTarget: innerElem })
      : null;
  ...
  _activate(element2, relatedElem) {
    ...
    this._queueCallback(
      complete,
      element2,
      element2.classList.contains(CLASS_NAME_FADE$2),
    );
  }
...
class Tab2 extends Tab$1 {
  ...
  _activate(element2, relatedElem) {
    ...
    this._queueCallback(
      complete,
      element2,
      element2.classList.contains(CLASS_NAME_FADE$1),
    );
  }
  _deactivate(element2, relatedElem) {
    if (!element2) {
...
(() => {
          ...
          if ("undefined" != typeof window)
            try {
              ...
              var t2 = function (e4, t3) {
                var n2, r;
                return (
                  ((t3 = t3 || {}).bubbles = !!t3.bubbles),
                  (t3.cancelable = !!t3.cancelable),
                  (n2 = document.createEvent("CustomEvent")).initCustomEvent(
                    e4,
                    t3.bubbles,
                    t3.cancelable,
                    t3.detail,
              ...
              (t2.prototype = window.Event.prototype),
                (window.CustomEvent = t2);
            }
        })();
      },
      379: (e2, t2, n2) => {
            ...
            return function (t3) {
              if (void 0 === e3[t3]) {
                ...
                if (
                  ...
                  n3 instanceof window.HTMLIFrameElement
                )
                  try {
        ...
        function u(e3, t3) {
          for (var n3 = {}, r2 = [], o2 = 0; o2 < e3.length; o2++) {
            var u2 = e3[o2],
              c2 = t3.base ? u2[0] + t3.base : u2[0],
              l2 = n3[c2] || 0,
              s2 = "".concat(c2, " ").concat(l2);
            n3[c2] = l2 + 1;
            var d2 = i(s2),
              f2 = { css: u2[1], media: u2[2], sourceMap: u2[3] };
            -1 !== d2
              ? (a[d2].references++, a[d2].updater(f2))
              : a.push({ identifier: s2, updater: m(f2, t3), references: 1 }),
              r2.push(s2);
          }
          return r2;
        }
        ...
        function d(e3, t3, n3, r2) {
          var o2 = n3
            ? ""
            : r2.media
              ? "@media ".concat(r2.media, " {").concat(r2.css, "}")
              : r2.css;
          if (e3.styleSheet) e3.styleSheet.cssText = s(t3, o2);
          else {
            var a2 = document.createTextNode(o2),
              i2 = e3.childNodes;
            i2[t3] && e3.removeChild(i2[t3]),
              i2.length ? e3.insertBefore(a2, i2[t3]) : e3.appendChild(a2);
          }
        }
        function f(e3, t3, n3) {
          var r2 = n3.css,
            ...
            a2 = n3.sourceMap;
          if (
            (o2 ? e3.setAttribute("media", o2) : e3.removeAttribute("media"),
            a2 &&
              "undefined" != typeof btoa &&
              (r2 +=
                "\n/*# sourceMappingURL=data:application/json;base64,".concat(
                  btoa(unescape(encodeURIComponent(JSON.stringify(a2)))),
                  " */",
                )),
            e3.styleSheet)
          )
        ...
        function m(e3, t3) {
          ...
          if (t3.singleton) {
            var a2 = p++;
            (n3 = v || (v = c(t3))),
              (r2 = d.bind(null, n3, a2, false)),
              (o2 = d.bind(null, n3, a2, true));
          } else
            (n3 = c(t3)),
              (r2 = f.bind(null, n3, t3)),
              (o2 = function () {
                !(function (e4) {
        ...
        e2.exports = function (e3, t3) {
          ...
          return function (e4) {
            if (
              ((e4 = e4 || []),
              "[object Array]" === Object.prototype.toString.call(e4))
            ) {
  ...
  (n.n = (e2) => {
    ...
    (() => {
      ...
      function a(e3) {
        e3.hasAttribute("autocompleted") &&
          (e3.removeAttribute("autocompleted"),
          e3.dispatchEvent(
      ...
      t2()(r.Z, { insert: "head", singleton: false }),
        r.Z.locals,
        n(810),
        document.addEventListener(
          "animationstart",
          function (e3) {
            "onautofillstart" === e3.animationName
              ? o(e3.target)
              : a(e3.target);
          },
          true,
        ...
        document.addEventListener(
          "input",
          function (e3) {
            "insertReplacementText" !== e3.inputType && "data" in e3
              ? a(e3.target)
              : o(e3.target);
          },
          true,
...
class Input extends BaseComponent2 {
  ...
  get input() {
    const inputElement =
      SelectorEngine$1.findOne("input", this._element) ||
      SelectorEngine$1.findOne("textarea", this._element);
    return inputElement;
  }
  // Public
  init() {
  ...
  _getNotchData() {
    this._notchMiddle = SelectorEngine$1.findOne(
  ...
  _applyDivs() {
    const allNotchWrappers = SelectorEngine$1.find(
      SELECTOR_NOTCH,
      this._element,
    );
  ...
  show() {
    ...
    if (this._config.parent) {
      activeChildren = this._getFirstLevelChildren(SELECTOR_ACTIVES)
        .filter((element2) => element2 !== this._element)
        .map((element2) =>
...
let Dropdown$1 = class Dropdown extends BaseComponent$1 {
  ...
  _selectMenuItem({ key, target }) {
    const items = SelectorEngine.find(
      ...
      this._menu,
    ).filter((element2) => isVisible(element2));
    if (!items.length) {
      return;
    }
    getNextActiveElement(
...
class Dropdown2 extends Dropdown$1 {
  ...
  _bindHiddenEvent() {
    EventHandler$1.on(this._parent, EVENT_HIDDEN, (e) => {
      const hiddenEvent = EventHandler$1.trigger(
        this._parent,
        EVENT_HIDDEN_MDB,
        {
          relatedTarget: e.relatedTarget,
...
class Range extends BaseComponent2 {
  ...
  _thumbUpdate() {
    ...
    const thumbValue = SelectorEngine$1.findOne(
...
const inputCallback = (component, initSelector) => {
  ...
  EventHandler$1.on(window, "shown.bs.modal", (e) => {
    SelectorEngine$1.find(SELECTOR_OUTLINE_INPUT, e.target).forEach(
      (element2) => {
    ...
    SelectorEngine$1.find(SELECTOR_OUTLINE_TEXTAREA, e.target).forEach(
      (element2) => {
        const instance = Input2.getInstance(element2.parentNode);
        if (!instance) {
          return;
  ...
  EventHandler$1.on(window, "shown.bs.dropdown", (e) => {
    ...
    if (target) {
      SelectorEngine$1.find(SELECTOR_OUTLINE_INPUT, target).forEach(
        (element2) => {
          const instance = Input2.getInstance(element2.parentNode);
          if (!instance) {
            return;
      ...
      SelectorEngine$1.find(SELECTOR_OUTLINE_TEXTAREA, target).forEach(
        (element2) => {
          const instance = Input2.getInstance(element2.parentNode);
          if (!instance) {
            return;
  ...
  EventHandler$1.on(window, "shown.bs.tab", (e) => {
    ...
    if (e.target.href) {
      ...
      targetId = Manipulator$1.getDataAttribute(e.target, "target").split(
        "#",
      )[1];
    }
  ...
  SelectorEngine$1.find(SELECTOR_DATA_INIT).map(
    (element2) => new Input2(element2),
  );
...
const rippleCallback = (component, initSelector) => {
  ...
  EventHandler$1.one(
    ...
    "mousedown",
    initSelector,
    Ripple2.autoInitial(new Ripple2()),
  );
};
